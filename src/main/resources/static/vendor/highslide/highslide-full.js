...
expandCursor : 'zoomin.cur',  restoreCursor : 'zoomout.cur',  expandDuration : 250,  restoreDuration : 250,
marginLeft : 15,
marginRight : 15,
marginTop : 15,
marginBottom : 15,
zIndexCounter : 1001,  loadingOpacity : 0.75,
allowMultipleInstances: true,
numberOfImagesToPreload : 5,
outlineWhileAnimating : 2,  outlineStartOffset : 3,  padToMinWidth : false,  fullExpandPosition : 'bottom right',
fullExpandOpacity : 1,
showCredits : true,  creditsHref : 'http: creditsTarget : '_self',
enableKeyListener : true,
openerTagNames : ['a', 'area'],  transitions : [],
transitionDuration: 250,
dimmingOpacity: 0,  dimmingDuration: 50,  
allowWidthReduction : false,
allowHeightReduction : true,
preserveContent : true,  objectLoadTime : 'before',  cacheAjax : true,  anchor : 'auto',  align : 'auto',  targetX: null,  targetY: null,
dragByHeading: true,
minWidth: 200,
minHeight: 200,
allowSizeReduction: true,  outlineType : 'drop-shadow',  skin : {
	controls:
...
keyHandler : function(e) {
	if (!e) e = window.event;
	if (!e.target) e.target = e.srcElement;  	if (typeof e.target.form != 'undefined') return true;  	if (!hs.fireEvent(hs, 'onKeyDown', e)) return true;
	var exp = hs.getExpander();
	
	var op = null;
	switch (e.keyCode) {
		case 70:  			if (exp) exp.doFullExpand();
			return true;
		case 32:  			op = 2;
			break;
...
wrapperMouseHandler : function (e) {
	try {
		...
		if (!e.target) e.target = e.srcElement;  		if (!e.relatedTarget) e.relatedTarget = 
			over ? e.fromElement : e.toElement;  		var exp = hs.getExpander(e.target);
...
hs.fx.prototype = {
	...
	custom: function(from, to, unit){
		...
		this.start = from;
		this.end = to;
		this.unit = unit; 		this.now = this.start;
...
htmlSizeOperations : function () {
    ...
    if (node && !/(iframe|swf)/.test(this.objectType)) {
    	var cNode = node;      	node = hs.createElement(cNode.nodeName, null, {overflow: 'hidden'}, null, true);
    	cNode.parentNode.insertBefore(node, cNode);
    	node.appendChild(hs.clearing);      	node.appendChild(cNode);
...
justify : function (p, moveOnly) {
	...
	if (tgt && hs.$(tgt)) {
		...
		if (p.pos + p.get('wsize') > p.scroll + p.clientSize - p.marginMax) {
			...
			} else {  				p.pos = p.scroll + p.marginMin;
...
crossfade : function (up, to, from) {
	...
	for (var n in names) {
		this[n] = names[n].content.cloneNode(!names[n].iframe);  		hs.setStyles(this[n], {
			position: 'absolute',
			border: 0,
			visibility: 'visible'
		});
	...
	hs.animate(wrapper, {
		...
		complete: function () {
			...
			exp.afterExpand();
			if (last.isHtml) last.htmlPrepareClose();  			last.afterClose();
			exp.last = null;
		}
...
showHideElements : function (tagName, visibility) {
	...
	for (var i = 0; i < els.length; i++) {
		if (prop == 'visibility' || (document.defaultView.getComputedStyle(
			...
			if (visibility == 'visible' && hiddenBy) {
			...
			} else if (visibility == 'hidden') {  				var elPos = hs.getPosition(els[i]);
				...
				if (!clearsX && !clearsY && wrapperKey != this.key) {  					if (!hiddenBy) {
...
htmlPrepareClose : function() {
	if (hs.geckoMac) {  		if (!hs.mask) hs.mask = hs.createElement('div', null, 
